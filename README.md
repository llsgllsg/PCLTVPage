...
- **影视占比**: 一张环形图卡片, 电视收视 / 院线票房各一个 SVG 环形图(用 WPF `Path` 的 SVG arc 语法绘制, 无需额外命名空间), 展示头部 TOP5 剧集的相对占比, 右侧带图例(色块 + 剧集 + 占比)。
- **热搜式排行**: 每个渠道一张卡片, 采用"微博热搜"式列表(排名 | 剧集名 | 热度值), 所有条目挤在同一张卡片内, 不再每个剧集单独一张卡片。
